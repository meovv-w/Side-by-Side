...
Page({
  ...
  onShow() { api.listSocial().then(res => { if (res.ok) this.setData({ following: res.data.following.map(item => ({ ...item, avatarText: item.targetType === 'team' ? '队' : (item.target.nickname || '同').slice(0, 1) })), followers: res.data.followers.map(item => ({ ...item, avatarText: (item.nickname || '同').slice(0, 1) })), blocked: res.data.blocked.map(item => ({ ...item, avatarText: (item.nickname || '同').slice(0, 1) })) }); }); },
