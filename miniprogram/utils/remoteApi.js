...
function connectRealtime() {
  ...
  socketTask.onMessage(event => {
    ...
    if (payload.event === 'notification' && payload.data && ['high', 'urgent'].includes(payload.data.priority)) {
      wx.showModal({ title: payload.data.title, content: payload.data.content, showCancel: false });
    }
...
async function getHome() {
  ...
  const center = currentRaw && currentRaw.route && currentRaw.route[0] || { lng: 120.1551, lat: 30.2741 };
  const [tripRows, productRows, conversationRows, topicRows, map] = await Promise.all([
    request('/api/trips'), request(`/api/products?lng=${center.lng}&lat=${center.lat}`),
    request('/api/conversations'), request(`/api/poi-topics?lng=${center.lng}&lat=${center.lat}`),
    request(`/api/map/context?lng=${center.lng}&lat=${center.lat}&city=330100`)
  ]);
  ...
  return {
    ...
    data: {
      ...
      weather: map.ok ? {
        altitude: Math.round(Number(((map.data.teammates || []).find(item => item.user.id === home.data.user.id) || {}).altitude || 0)),
...
async function updateTrip(tripId, payload) {
  const response = await request(`/api/trips/${tripId}`, { method: 'PATCH', data: {
    title: payload.title, teamName: payload.teamName, departAt: payload.departAt,
    days: Number(payload.days), dailyKm: Number(payload.dailyKm), maxCars: Number(payload.seatTotal),
    priceShare: Number(payload.priceShare), depth: depthToApi(payload.depth), plans: payload.plans,
    equipment: payload.equipment, privacy: payload.privacy, discoverable: payload.discoverable, note: payload.note
  } });
  return response.ok ? { ok: true, data: adaptTrip(response.data) } : response;
}
...
async function getPrivateChat(userId) {
  ...
  return { ok: true, data: {
    ...
    blocked: response.data.relation.type === 'blocked', canSend: response.data.canSend,
...
async function getPoiChat(topicId) {
  ...
  if (!response.ok) return response;
  return { ok: true, data: { room: adaptTopic({ ...response.data.topic, membership: response.data.membership, participantCount: response.data.participantCount }), messages: (response.data.messages || []).map(adaptMessage) } };
}
...
async function sendPoiMessage(topicId, content, type = 'text') {
  let mediaUrl = '';
  ...
  const response = await request(`/api/poi-topics/${topicId}/messages`, { method: 'POST', data: { content, mediaUrl, type } });
  return response.ok ? { ok: true, data: adaptMessage(response.data) } : response;
}

async function listGroupbuys() {
  const response = await request('/api/products?sort=hot');
  return response.ok ? { ok: true, data: flattenGroupbuys(response.data) } : response;
}
...
async function getGroupbuy(sessionId) {
  ...
  if (!response.ok) return response;
  return { ok: true, data: adaptGroupbuy(response.data.session, response.data.product, response.data.merchant, response.data.participants, couponResponse.ok ? couponResponse.data : []) };
}
...
async function getMerchant(merchantId) {
  ...
  return { ok: true, data: {
    ...
    groupbuys: flattenGroupbuys(response.data.products || [])
  } };
}
...
async function getMine() {
  ...
  return { ok: true, data: {
    user: adaptUser(profile.data), trips: tripRows.ok ? tripRows.data.filter(item => item.joined || item.owned).map(adaptTrip) : [],
...
async function getUserProfile(userId) {
  ...
  return { ok: true, data: { user: adaptUser(profile.data), relation: profile.data.relation, following, trips: (profile.data.trips || []).map(adaptTrip) } };
}
...
async function listInvites() {
  const [summary, share, profile] = await Promise.all([
    request('/api/invites/me'), request('/api/invites/share', { method: 'POST', data: { source: 'link' } }), request('/api/users/me')
  ]);
  if (!summary.ok) return summary;
  return { ok: true, data: {
    inviteCode: share.ok ? share.data.inviteCode : '', sharePath: share.ok ? share.data.miniProgramPath : '/pages/login/login', qrCode: share.ok ? share.data.qrCode : '',
    records: (summary.data.items || []).map(item => ({ ...item, _id: item.id, inviteeName: item.invitee.nickname, status: item.status === 'registered' ? 'registered' : 'ordered', bonus: item.status === 'registered' ? 10 : 30 })),
    rewards: [{ target: 1, title: '首位好友', reward: '¥5 奖励券' }, { target: 3, title: '同行小队', reward: '¥10 奖励券' }, { target: 5, title: '路线召集人', reward: '¥20 奖励券' }, { target: 10, title: '同路领航员', reward: '¥50 奖励券' }].map(item => ({ ...item, reached: Number(summary.data.stats.firstOrders || 0) >= item.target })),
    stats: summary.data.stats,
    canBindInviter: Boolean(profile.ok && !profile.data.invitedBy && Date.now() - new Date(profile.data.createdAt).getTime() <= 7 * 86400000)
  } };
}
...
async function createNextTrip(payload) {
  ...
  const response = await request('/api/trip-drafts', { method: 'POST', data: {
    ...
    departAt: payload.departAt, waypoints: [], note: payload.note || ''
  } });
...
async function resolveTripPoints(payload) {
  if (payload.startPoint && payload.endPoint) return { ok: true, data: { start: payload.startPoint, end: payload.endPoint } };
  const fallback = { start: { lng: 120.1551, lat: 30.2741 }, end: { lng: 119.0419, lat: 29.6097 } };
  const from = payload.from || payload.startName;
  const to = payload.to || payload.endName;
  const [start, end] = await Promise.all([request(`/api/map/geocode?address=${encodeURIComponent(from)}`), request(`/api/map/geocode?address=${encodeURIComponent(to)}`)]);
  if (!start.ok || !end.ok) return { ok: false, message: (start.message || end.message || '地点解析失败') };
  return { ok: true, data: {
    start: geocodePoint(start.data) || fallback.start,
    end: geocodePoint(end.data) || fallback.end
  } };
}
...
function tripRequest(payload, points) {
  return {
    ...
    endName: payload.to, endLng: points.end.lng, endLat: points.end.lat, departAt: payload.departAt,
    days: Number(payload.days), dailyKm: Number(payload.dailyKm), maxCars: Number(payload.seatTotal),
...
function adaptTrip(trip = {}) {
  const status = { recruiting: 'open', full: 'full', started: 'open', completed: 'done', cancelled: 'done' }[trip.status] || trip.status;
  return {
    ...
    teammates: (trip.teammates || []).map(item => ({ userId: item.userId || item.user_id, nickname: item.nickname, latitude: Number(item.latitude), longitude: Number(item.longitude) })),
    joined: trip.joined, owned: trip.owned, requestStatus: trip.applicationStatus || trip.requestStatus || 'none', matchRate: trip.matchRate,
...
function adaptTopic(topic) {
  return {
    ...topic, _id: topic.id, online: topic.participantCount || 0, lastMessage: topic.latestMessage && topic.latestMessage.content || '',
...
function flattenGroupbuys(products) {
  const result = [];
...
function adaptGroupbuy(session, product, merchant, participants = [], coupons = []) {
  return {
    ...
    originPrice: Number(product.originPrice), currentPrice: Number(session.currentPrice), price: Number(session.currentPrice),
...
function adaptOrder(order) {
  ...
  return {
    ...
    verifyCode: order.verifyCode, createdAt: normalizeDate(order.createdAt), paidAt: normalizeDate(order.paidAt), usedAt: normalizeDate(order.verifiedAt)
  };
}
...
function adaptCoupon(instance) {
  ...
  return {
    ...
    threshold: Number(coupon.thresholdAmount || 0), type: coupon.type, status: instance.status,
...
function mapLayers(context) {
  ...
  for (const item of context.otherTeams || []) rows.push({
    ...
    subtitle: item.direction === 'opposite' ? '逆向车队' : '同向车队',
    desc: `队长 ${item.leader.nickname} · Lv.${item.leader.level} · 距你 ${(item.distanceMeters / 1000).toFixed(1)}km`,
  ...
  for (const item of context.soloDrivers || []) rows.push({
    type: 'driver', title: item.user.nickname, subtitle: `个人自驾者 · Lv.${item.user.level}`,
    desc: `距你 ${(item.distanceMeters / 1000).toFixed(1)}km · ${item.user.bio || '正在附近自驾'}`,
    userId: item.user.id, latitude: Number(item.location.lat), longitude: Number(item.location.lng),
    unread: Number(item.unreadPrivateCount || 0), preview: item.latestPrivateMessage && item.latestPrivateMessage.content || ''
  });
  for (const item of context.amap && context.amap.services || []) rows.push({ type: 'poi', title: item.name, subtitle: item.type, desc: item.address, phone: item.tel, latitude: item.lat, longitude: item.lng });
  ...
  for (const item of context.trafficEvents || []) rows.push({ type: 'traffic', targetId: item.id, title: item.title, subtitle: '实时路况', desc: item.description, latitude: Number(item.lat), longitude: Number(item.lng) });
  return rows;
}
...
function normalizeDate(value) { return value ? String(value).replace('T', ' ').replace(/\.000Z$/, '').replace(/Z$/, '') : ''; }
...
module.exports = {
  isEnabled, login, loginWechat, sendSmsCode, loginWithPhone, getHome, listTrips, getTrip, createTrip, planRoute, updateTrip,
  ...
  getPoiChat, createPoiChat, followPoiChat, sendPoiMessage, listGroupbuys, getGroupbuy, getMerchant, createGroupbuySession,
  createOrder, listOrders, getOrder, requestRefund, getMine, updateProfile, getUserProfile, getBadgeWall,
  toggleFollow, setBlocked, listSocial, getCertification, startCertification, submitCertification,
  listCoupons, listInvites, bindInviterByPhone, createNextTrip, matchNextTrip, publishNextTrip, submitTicket, listTickets,
  getSettings, updateSettings, recordEmergency, reportSafetyEvent, getAdminSnapshot, resetDemo, subscribeRealtime
};
